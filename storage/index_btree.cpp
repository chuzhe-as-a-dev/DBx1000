...
RC index_btree::make_node(uint64_t part_id, bt_node*& node) {
  ...
  new_node->keys =
      (idx_key_t*)mem_allocator.alloc((order - 1) * sizeof(idx_key_t), part_id);
  new_node->pointers =
...
// transitions to NONE only when the count reaches 0 (last shared reader).
// (AI-generated)
...
// Returns Abort if other readers hold a SH latch; the caller must back off.
// (AI-generated)
...
//     On any upgrade failure the traversal aborts and the caller retries.
//     (AI-generated)
RC index_btree::find_leaf(glob_param params, idx_key_t key,
                          idx_acc_t access_type, bt_node*& leaf,
                          bt_node*& last_ex) {
...
// which continues propagation until a non-full ancestor is found.
// (AI-generated)
