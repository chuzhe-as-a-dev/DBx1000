...
void Row_mvcc::init(row_t* row) {
  ...
  _write_history =
      (WriteHisEntry*)_mm_malloc(sizeof(WriteHisEntry) * _his_len, 64);
...
void Row_mvcc::buffer_req(TsType type, txn_man* txn, bool served) {
  ...
  while (true) {
    for (uint32_t i = 0; i < _req_len; i++) {
      ...
      // in some sense, only need to keep the served read request with the max
      // ts.
      //
...
void Row_mvcc::double_list(uint32_t list) {
  if (list == 0) {
    WriteHisEntry* temp =
        (WriteHisEntry*)_mm_malloc(sizeof(WriteHisEntry) * _his_len * 2, 64);
    for (uint32_t i = 0; i < _his_len; i++) {
    ...
    for (uint32_t i = _req_len; i < _req_len * 2; i++) temp[i].valid = false;
    _mm_free(_requests);
    _requests = temp;
    _req_len = _req_len * 2;
  }
...
RC Row_mvcc::access(txn_man* txn, TsType type, row_t* row) {
  ...
  for (uint32_t i = 0; i < _req_len; i++)
    if (_requests[i].valid) {
      ...
      if (_exists_prewrite) assert(_prewrite_ts < _requests[i].ts);
    }
#endif
  if (type == R_REQ) {
    if (ts < _oldest_wts)
      ...
      if (_exists_prewrite && _prewrite_ts < ts) {
        // exists a pending prewrite request before the current read. should
        // wait.
        rc = WAIT;
        ...
        if (ts > _max_served_rts) _max_served_rts = ts;
      }
    } else {
      rc = RCOK;
      // ts is between _oldest_wts and _latest_wts, should find the correct
      // version
      uint32_t the_ts = 0;
      uint32_t the_i = _his_len;
      for (uint32_t i = 0; i < _his_len; i++) {
        if (_write_history[i].valid && _write_history[i].ts < ts &&
    ...
    if (ts < _latest_wts || ts < _max_served_rts ||
...
//   4. len >= g_thread_cnt: evict the oldest version (force-recycle).
//   (AI-generated)
row_t* Row_mvcc::reserveRow(ts_t ts, txn_man* txn) {
  ...
  if (_oldest_wts < min_ts && _num_versions == _his_len) {
    ...
    if (idx != _his_len) {
      ...
      for (uint32_t i = 0; i < _his_len; i++) {
        if (_write_history[i].valid && _write_history[i].ts <= max_recycle_ts) {
          _write_history[i].valid = false;
          _write_history[i].reserved = false;
  ...
  for (uint32_t i = 0; i < _his_len; i++)
    if (_write_history[i].valid) {
      his_size++;
      if (_write_history[i].ts > max_ts) max_ts = _write_history[i].ts;
    }
  assert(his_size == _num_versions);
  if (_num_versions > 0) assert(max_ts == _latest_wts);
#endif
  uint32_t idx = _his_len;
  // _write_history is not full, find an unused entry for P_REQ.
  if (_num_versions < _his_len) {
    for (uint32_t i = 0; i < _his_len; i++) {
      if (!_write_history[i].valid && !_write_history[i].reserved &&
          _write_history[i].row != NULL) {
        idx = i;
        break;
      } else if (!_write_history[i].valid && !_write_history[i].reserved)
        idx = i;
    }
    assert(idx < _his_len);
  }
  ...
  if (idx == _his_len) {
    if (_his_len >= g_thread_cnt) {
      // all entries are taken. recycle the oldest version if _his_len is too
      // long already
...
void Row_mvcc::update_buffer(txn_man* txn, TsType type) {
  ...
  for (uint32_t i = 0; i < _req_len; i++)
    if (_requests[i].valid && _requests[i].type == P_REQ &&
        _requests[i].ts > ts && _requests[i].ts < next_pre_ts)
      next_pre_ts = _requests[i].ts;
  // return all pending quests between txn->ts and next_pre_ts
  for (uint32_t i = 0; i < _req_len; i++) {
    if (_requests[i].valid) assert(_requests[i].ts > ts);
    // return pending R_REQ
    if (_requests[i].valid && _requests[i].type == R_REQ &&
        _requests[i].ts < next_pre_ts) {
      if (_requests[i].ts > _max_served_rts) _max_served_rts = _requests[i].ts;
      _requests[i].valid = false;
