...
//                waiters are kept sorted by timestamp so older txns queue
//                ahead. Conflict is also forced if any earlier-timestamped
//                waiter exists, to prevent starvation of the queue head.
...
//                Every new waiter goes to the tail; the caller drives
...
final:
  ...
  if (rc == WAIT && CC_ALG == DL_DETECT) {
    ...
    txnids = (uint64_t*)mem_allocator.alloc(
        sizeof(uint64_t) * (owner_cnt + waiter_cnt), part_id);
    txncnt = 0;
...
LockEntry* Row_lock::get_entry() {
  LockEntry* entry =
      (LockEntry*)mem_allocator.alloc(sizeof(LockEntry), _row->get_part_id());
  return entry;
}
