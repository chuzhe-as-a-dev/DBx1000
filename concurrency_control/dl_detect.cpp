...
bool DL_detect::nextNode(uint64_t txnid, DetectData* detect_data) {
  ...
  for (std::list<uint64_t>::iterator i = dependency[thd].adj.begin();
  ...
  detect_data->recStack[thd] = false;
  if (detect_data->loop && detect_data->onloop &&
...
int DL_detect::detect_cycle(uint64_t txnid) {
  ...
  DetectData* detect_data =
      (DetectData*)mem_allocator.alloc(sizeof(DetectData), thd);
  detect_data->visited = (bool*)mem_allocator.alloc(sizeof(bool) * V, thd);
  detect_data->recStack = (bool*)mem_allocator.alloc(sizeof(bool) * V, thd);
  for (int i = 0; i < V; i++) {
