...
struct DepThd {
  ...
  char pad[2 * CL_SIZE - sizeof(int64_t) - sizeof(pthread_mutex_t) -
           sizeof(std::list<uint64_t>) - sizeof(int)];
};
...
class DL_detect {
 ...
 private:
  ...
  bool isCyclic(uint64_t txnid,
                DetectData* detect_data);  // return if "thd" is causing a cycle
};
