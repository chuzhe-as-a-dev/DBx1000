...
// Non-atomic path acquires trylock to prevent a concurrent write from racing.
// (AI-generated)
...
#if ATOMIC_WORD
  uint64_t v = _tid_word;
  M_ASSERT(tid > (v & (~LOCK_BIT)) && (v & LOCK_BIT),
           "tid=%ld, v & LOCK_BIT=%ld, v & (~LOCK_BIT)=%ld\n", tid,
           (v & LOCK_BIT), (v & (~LOCK_BIT)));
...
void Row_silo::lock() {
  ...
  uint64_t v = _tid_word;
  while ((v & LOCK_BIT) ||
         !__sync_bool_compare_and_swap(&_tid_word, v, v | LOCK_BIT)) {
    PAUSE
    v = _tid_word;
  }
#else
