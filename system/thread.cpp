...
// with a cooldown delay before retrying, avoiding hot-retry storms on contended
// rows. (AI-generated)
...
//   Priority 1 – pick a cooled-down query from the abort buffer (past
//   ready_time). Priority 2 – if buffer is full, sleep until the earliest
//   ready_time. Priority 3 – fetch a fresh query from the pre-generated queue.
//
...
// delay; without the buffer the thread simply sleeps for the penalty.
// (AI-generated)
RC thread_t::run() {
#if !NOGRAPHITE
      ...
      if (_abort_buffer_enable) {
        ...
        while (trial < 2) {
          ...
          if (_abort_buffer_empty_slots < _abort_buffer_size) {
            for (int i = 0; i < _abort_buffer_size; i++) {
              if (_abort_buffer[i].query != NULL &&
                  curr_time > _abort_buffer[i].ready_time) {
          ...
          if (m_query == NULL && _abort_buffer_empty_slots == 0) {
            assert(trial == 0);
            M_ASSERT(min_ready_time >= curr_time,
                     "min_ready_time=%ld, curr_time=%ld\n", min_ready_time,
                     curr_time);
